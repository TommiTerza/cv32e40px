# cvt40 SIMT Prototype

This repository contains a modified version of the OpenHW Group CORE‑V cvt40 32‑bit in‑order RISC‑V core.
...
- **Warped fetch and PC tracking**
  ...
  - Per‑warp program counters and state machines in `cvt40_core.sv`; warp 0 starts active, others start inactive.
  - A simple round‑robin warp scheduler (`cvt40_warp_scheduler.sv`) integrated into the IF stage
    (`cvt40_if_stage.sv`) to choose the next warp to fetch from.
  - Per‑warp stall feedback from the rest of the pipeline back into IF to avoid selecting blocked warps.

- **SIMT ISA hooks**
  - New SIMT opcodes in `cvt40_pkg.sv`:
  ...
  - These are encoded as `OPCODE_CUSTOM_0` instructions with dedicated `funct7` values and decoded in
    `cvt40_decoder.sv`.
  - The decode stage (`cvt40_id_stage.sv`) forwards SIMT metadata (`simt_valid`, `simt_op`, `simt_rs1_ex_o`,
    `simt_rs2_ex_o`, `wid_ex_o`) down to the execute stage.
  - The execute stage (`cvt40_ex_stage.sv`) converts these into SIMT commands
...
- **Multi‑warp register file**
  ...
  - The ID stage (`cvt40_id_stage.sv`) passes the current `wid` into the RF and extends its hazard/forwarding
    logic so that dependencies are only tracked within a warp (no cross‑warp hazards).
  - The EX stage (`cvt40_ex_stage.sv`) propagates warp IDs through the EX/WB pipeline, including multicycle
    APU operations, so that late writes are still directed to the correct warp bank.

- **Hardware loops and other state**
  - Hardware loop registers (`cvt40_hwloop_regs.sv`) already support per‑warp state; this design keeps that
    structure and feeds them with the appropriate warp ID from the pipeline.

- **Issue‑stage and scoreboard prototype**
  - A standalone issue stage (`cvt40_is_stage.sv`) implements:
...
- **Source:** `cvt40/example_tb/core/custom/simt_vector_add.c`
- **Behaviour:**
...
1. Build the RTL testbench (once) and the SIMT vector‑add firmware:
   - `cd cvt40/example_tb/core`
   - `make vector-add-vsim-run`

2. The `vector-add-vsim-run` target will:
   - Compile the RTL testbench and core according to `cvt40/example_tb/core/Makefile`.
...
`CV_CORE_MANIFEST` (and any related file lists) at the cvt40/SIMT RTL you want to test.
...
- The SIMT machinery (multi‑warp PC/state, SIMT instructions, multi‑warp RF, basic test firmware) is functional
  ...
  - The `cvt40_is_stage` scoreboard/issue block is not yet fully wired into the main pipeline.

- As with the original cvt40, this repository is not intended to be a silicon‑ready drop‑in. Treat the
